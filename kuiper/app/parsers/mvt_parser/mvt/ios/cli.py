...
from mvt.common.help import HELP_MSG_FAST, HELP_MSG_IOC,HELP_MSG_LIST_MODULES, HELP_MSG_MODULE,HELP_MSG_OUTPUT
...
@click.option("--output", "-o", type=click.Path(exists=True), help=HELP_MSG_OUTPUT)
...
def check_backup(ctx, iocs, output, fast, backup_path, list_modules, module):
    ...
    timeline_detected = []
    for backup_module in BACKUP_MODULES + MIXED_MODULES:
        if module and backup_module.__name__ != module and module != "Timeline":
            continue
        ...
        TimelineOnly = (module == "Timeline")
        run_module(m , TimelineOnly = TimelineOnly )
...
# call the cli object, not need to install the mvt
